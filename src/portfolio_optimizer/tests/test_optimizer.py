...
def case_optimizer():
    ...
    port = Portfolio(date='2018-02-19',
                     ...
                     positions=pos)
    save_cut_off = portfolio_optimizer.portfolio.VOLUME_CUT_OFF
    # Все кейсы составлены для константы ограничения на объем 0.0024
    portfolio_optimizer.portfolio.VOLUME_CUT_OFF = 0.0024
    opt = Optimizer(port)
    # Все кейсы составлены для константы сглаживания 0.9
    opt.returns_metrics._decay = 0.9

    yield opt
    portfolio_optimizer.portfolio.VOLUME_CUT_OFF = save_cut_off
